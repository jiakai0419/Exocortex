...
下一次实际改代码时，建议继续 Phase 1 的第二刀：

1. 扩大 `tsconfig.json` 的 include 范围到 `scripts/lib/lark-im-adapter.mjs`。
2. 为 adapter result、Lark API envelope、people context 增加 JSDoc 类型。
3. 让 `npm run typecheck` 继续保持 no-emit、无 runtime 影响。
4. 暂时不迁移文件、不引入 build、不改 LaunchAgent。

这一刀完成后，再考虑 `lark-im-worker-core` 和 `sync-status-core`。
