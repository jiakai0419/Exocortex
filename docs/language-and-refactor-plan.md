...
4. `src/cli/lark-im-sync-command.mjs` 已承载 `lark-im-sync` CLI 行为；`sync-status` 和 `doctor` 已拆成 report / view / command 三层；`lark-im-service status` 已拆成 report / view 两层；`lark-im-worker` CLI 外壳已迁入 `src/cli/lark-im-worker-command.mjs`。下一步先观察 worker、CI 和这几个诊断入口，再考虑是否迁移 `lark-im-service` 生命周期命令，不要急着做 TypeScript production CLI rewrite。
