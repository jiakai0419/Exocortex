...
下一次实际改代码时，建议继续 Phase 1 的第五刀：

1. 扩大 `tsconfig.json` 的 include 范围到低风险 CLI 入口：`scripts/help.mjs`、`scripts/doctor.mjs`、`scripts/sync-status.mjs`。
2. 为 CLI option、render payload、JSON report 增加 JSDoc 类型。
3. 让 `npm run typecheck` 继续保持 no-emit、无 runtime 影响。
4. 暂时不迁移文件、不引入 build、不改 LaunchAgent。

这一刀完成后，再考虑 maintenance / probe 脚本是否值得进入 typecheck，或进入 Phase 2 的目录边界整理。
