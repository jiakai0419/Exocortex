...
4. `src/cli/lark-im-sync-command.mjs` 已承载 `lark-im-sync` CLI 行为；`sync-status`、`doctor` 和 `messages` 已拆成 report / view / command 三层；`lark-im-service` 和 `lark-im-worker` CLI 外壳已迁入 `src/cli`。下一步先观察 worker、CI 和这些入口的稳定性，不要急着做 TypeScript production CLI rewrite。
