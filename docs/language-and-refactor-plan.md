...
下一次实际改代码时，建议继续 Phase 1 的第三刀：

1. 扩大 `tsconfig.json` 的 include 范围到 `scripts/lib/lark-im-worker-core.mjs` 和 `scripts/lib/sync-status-core.mjs`。
2. 为 worker cycle、worker event、health summary 增加 JSDoc 类型。
3. 让 `npm run typecheck` 继续保持 no-emit、无 runtime 影响。
4. 暂时不迁移文件、不引入 build、不改 LaunchAgent。

这一刀完成后，再考虑 `doctor-core` 和 `terminal`。
