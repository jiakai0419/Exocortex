...
- `sqlite-maintenance prune-runs` 的 dry-run 默认行为、旧 succeeded no-op run 清理规则、不删除 failed/running/current success 的边界，以及 apply 时通过全局 maintenance lock 和 worker 写入互斥。
