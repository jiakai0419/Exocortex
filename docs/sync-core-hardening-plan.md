...
v0 质量验收已经完成并固化到 `docs/v0-baseline.md`。下一轮建议先做 24 小时长期运行观察，再处理 P2P sent 接收人字段审计；不急于进入 UI、语义层或新 Source。
...
- 真实库当前基线：
  - initial discovery 完成，112 页。
  - received scopes 4478 个，0 个缺 cursor。
  - reconcile 完成，56 页。
  - doctor OK，数据质量 0 missing sender names / 0 missing chat names / 0 invalid bodies。
  - `npm test` 通过 39 个测试。
...
- 历史回溯结果：
  - Application API 成功 0 个。
  - 权限不足 app_id 2 个。
  - fallback 解析 app_id 2 个。
  - 覆盖历史 app sender 消息 6 条。
  - ambiguous fallback 0 个。
- 新增 `docs/v0-baseline.md`，记录 v0 的保证、非保证、验收命令和下一步观察计划。
- `npm test` 当前通过 43 个测试。
